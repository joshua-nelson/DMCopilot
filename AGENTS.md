...
- If two approaches exist, present both with tradeoffs. Do not pick one silently. Exception: trivial tasks (typo, rename a local variable, add a log line) where the diff fits in one sentence.
...
- Language and version: `TODO` (no language files detected in repository root)
- Framework(s): `TODO`
- Package manager: `TODO` (no package.json / pyproject.toml / Cargo.toml / Gemfile / package-lock.json found)
- Runtime / deployment target: `TODO`

### Commands

- Install: `TODO` (no detected package manager files)
- Build: `TODO` (no detected build config: Makefile, pyproject.toml, Cargo.toml)
- Test (all): `TODO`
...
- Source lives in: `TODO` (no standard source directories detected)
- Tests live in: `TODO` (no `test` or `tests` directories detected)
